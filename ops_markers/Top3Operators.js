...
for (var i = 0; i < wells.length; i++) {
  ...
  if (wells[i].OperatorName === "Pioneer Natural Resources") {
    ...
    L.circle([wells[i].Latitude, wells[i].Longitude], {
      radius: 500, color: color, opacity:.5})
    // Adjust the radius.
    .bindPopup(`<h1>${wells[i].CountyName}</h1> <hr> <h3>${wells[i].WellName}</h3>`)
    .addTo(myMap);
  }
